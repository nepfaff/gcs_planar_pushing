...
class ProblemGenerator():
    def __init__(self,
        ...
        assert(self.workspace_radius > self.object_max_radius + self.robot_max_radius), "Workspace must be larger than the sum of the object and robot radii."
    ...
    def generate_initial_positions(self, n_samples=100):
        ...
        while count != n_samples:
            for i in range(dim_object):
                object_pos[count, i] = np.random.uniform(object_bounds[i][0], object_bounds[i][1])
            for i in range(dim_robot):
                robot_pos[count, i] = np.random.uniform(robot_bounds[i][0], robot_bounds[i][1])
                                   
            self.plant.SetPositions(self.plant_context, self.robot.model_instance(), robot_pos[count])
            if not ProblemGenerator.is_body_in_contact(
                ...
                print(f"Rejected sample {count}, robot pos: {robot_pos[count]}, object pos: {object_pos[count]}")
    ...
    def is_body_in_contact(body_of_interest, contact_results, print_result=False):
        ...
        for i in range(contact_results.num_point_pair_contacts()):
            ...
            if (
                ...
                force_string = np.array2string(info.contact_force(), formatter=formatter)
                if print_result:
                    print(
