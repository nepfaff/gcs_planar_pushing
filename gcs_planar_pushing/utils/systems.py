...
class ExternalForceSystem(LeafSystem):
    ...
    def DoCalcAbstractOutput(self, context, output):
        ...
        spatial_force.body_index = self._target_body_index
        spatial_force.p_BoBq_B = self._wrench_application_point
        spatial_force.F_Bq_W = SpatialForce(tau=self._wrench[3:], f=self._wrench[:3])
